...
点击“**传感器**”，寻找到“**BMP388气压传感器**”，“**DHTsensor**”，“**旋转编码器**”，“**超声波传感器**”，“**Gesture Recognition**”和“**RFID RC522 IIC**”并且分别点击选中。

![图片不存在](./media/9e24aed7fc10434434237d5e209ec2c0.png)

点击“**显示器**”，寻找到“**4位数码管TM1650_IIC**”，“**8*8点阵 IIC**”和“**NeoPixel彩灯**”并且分别点击选中。

![图片不存在](./media/30fbf7e8f65c6b858a98b12a73eb3748.png)
...
![图片不存在](./media/b1c844493b930127c6b1087a6e00e0cb.png)
...
![图片不存在](media/c96419d788df68b73eecb6c3c42b7f4e.png)
...
![图片不存在](./media/16ae5644e37c84d451000ecf4c1ddb04.png)
...
![图片不存在](media/e95131bd9a34c93ec260f6ff4e6a1efc.png)

![图片不存在](media/3942741fea503557e7747b323c2f4e2a.png)

![图片不存在](media/91f801ac526d5f4338b78207b5ce93d3.png)
...
激光头模块上的激光头发出的激光与常见的光不同。一方面，激光的单色性好。另一方面，激光发射器内部特定的结构，使得激光能够被聚集成单束光，朝着同一方向射出，亮度高，方向性好。
...
激光模块上主要由激光头组成，激光头由发光管芯、聚光透镜、铜可调套筒三部分组成。
...
光折断模块上的光电开关是是利用被检测物体对光束的遮挡或反射，由同步回路选通电路，从而检测遮挡物体的有无。所有能反射光线的物体都可以被检测。光电开关将输入的电流在发射器上转换为光信号并射出，然后接收器根据接收到的光线强弱或有无，对目标物体进行检测。
...
![图片不存在](./media/85bcf6bd89a9d1d362ba63cd4a74e649.png)
...
![图片不存在](media/1da5a959300a3e246efb0ebd3faacaae.png)
...
![图片不存在](./media/9bb3a5ab1ab82a276d8fe67e21305ddd.png)
...
![图片不存在](media/939103f5e7b03e03e9c25a2f8da2a459.png)
...
![图片不存在](./media/fb0b2529b34814e076779af8fe2e3c1b.png)
...
![图片不存在](./media/171f9bca40144717b187eb0754500382.png)
...
![图片不存在](media/7e13bb3ffa2b2bfd972e571e4017490c.png)
...
火焰传感器对火焰光谱特别灵敏，且灵敏度可调。性能稳定，是救火机器人的必备部件。火焰传感器上有一个远红外火焰探头，起着非常重要的作用，它可以用作机器人的眼睛来寻找火源或足球。利用它可以制作灭火机器人、足球机器人等。
...
![图片不存在](./media/7265f2bb81771569a07e5157df8be224.png)
...
你看过游戏手柄吗？游戏手柄上有按键，还有摇杆。摇杆是什么工作原理呢？在这个套件中，就有一个摇杆模块，它的主要元件是PS2手柄摇杆。控制时，我们需要将模块的X引脚和Y引脚连接至单片机的模拟口。B引脚连接至单片机数字口，V引脚接至单片机电源输出端（3.3-5V），G引脚接单片机GND。通过读取两个模拟值和一个数字口的高低电平情况，可以判断模块上摇杆的工作状态。
...
第02课学习了交通灯模块，利用高低电平对模块的3个LED进行点亮和熄灭实现交通灯的效果。我们这个套件中，还有一个SK6812 RGB模块，它是一个集控制电路与发光电路于一体的智能外控LED光源。每个LED原件其外型与一个5050LED灯珠相同，每个元件即为一个像素点，我们这个模块上有四个灯珠即四个像素点。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**21_SK6812_RGB**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/17b0389fbfccdc5d598941f78efed5a9.png)
...
![图片不存在](media/25400c9b8db8837beac707e454d950b8.png)
...
![图片不存在](./media/01b399c3ebef6a94c9d4503ebd259c22.png)

设置SK6812 RGB的管脚和点亮RGB灯珠的个数。

![图片不存在](./media/4a8367012ce57c2a674bc1513c475c79.png)

设置SK6812 RGB的亮度。

![图片不存在](./media/169ede89965f16b9bfc2f78125559f02.png)

熄灭所有的灯珠。

![图片不存在](./media/1b96ffbbe2bb349c46ba144380fde536.png)

从某个像素（灯珠）开始用某种颜色填充多个灯珠。

![图片不存在](./media/3bdea1a8b9a1fa2a504f023a7fec7608.png)

刷新点亮RGB。

![图片不存在](./media/7bab41284ff672b43d482af16aa51b0a.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**22_Encoder**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/d0142f1ea4dcf82681b077e208f955e8.png)
...
![图片不存在](media/91cb0a575706d60226e413f51de591de.png)

---

**8. 代码说明**

![图片不存在](./media/6f4cf691ad830006987495c6729db7c6.png)

声明编码器的 CLK、DT 和 SW 引脚所连接的 Arduino 引脚。

![图片不存在](./media/c138022e5f3969004e8a23e76ad2ff42.png)
...
![图片不存在](./media/9875dbef4ea6a522af450f588aa8b355.png)

然后将结果打印到串行监视器。

![图片不存在](./media/08d861873ec839520cc1b927fa92f025.png)

在if声明之后，我们更新lastStateCLKCLK 的当前状态。

![图片不存在](./media/b2c9443e644fcf7c20fe05d8087bb78f.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**23.1_Servo**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**23.2_Servo**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**24_Ultrasonic**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**25_IR_Receiver**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**26_XHT11**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**27_Four_Digita_Tube**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**28_8×8_Dot_Matrix**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**29_TFT_Screen**"。
...
RFID刷卡模块上内置的RFIDRFID-RC522射频模块采用的是Philips MFRC522原装芯片设计的读卡电路，使用方便，成本低廉，适用于设备开发、读卡器开发等高级应用的用户，也适用于需要进行射频卡终端设计/生产的用户。本模块可直接装入各种读卡器模具，通过IIC接口简单的2条线就可以直接与用户任何CPU主板或单片机相连接通信。

实验中用刷卡模块读取到的数据是4个16进制数，我们把这四个16进制数串以字符串的形式打印出来。例如，如果读取到的IC卡的数据为：0xED、0xF7、0x94、0x5A，则串口监视器显示出的信息字符串就是ED F7 94 5A；如果读取的钥匙扣的数据为：0x4C、0x09、0x6B、0x6E，则串口监视器打印出来的信息字符串就是4C 09 6B 6E。有时候看到的不是两位数，是因为前面有个0省略了，如0a它显示的就是a。不同的IC卡和钥匙扣，其数据是不一样的。
...
| ![图片不存在](media/6a82ee5dd8727ea183dc758cc34e30e0.png)   |![图片不存在](media/8cc701956c4d1ce4ce501fe370e0fbaa.png) |![图片不存在](media/5cac63a19ecafff8be1ddba2b3fdc015.png)    |
| ------------------------ | ------------------------ | ---------------------------- |
|主板 x1  | RFID刷卡模块 x1    |IC卡/钥匙扣 x1| 
...
![图片不存在](media/9d700bff7c304433caccfc340deecd70.png)

---

**6. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**30_RFID_RC522**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/0b1a8fcf0b30e48d191fbe70ed0ccdf9.png)
...
代码上传成功后，拔下USB线断电。按照接线图正确接好模块后再用USB线连接到计算机上电，打开串口监视器，设置波特率为 **<u>9600</u>**。用IC卡和钥匙扣靠近RFID模块，串口监视器打印出RFID刷卡模块读取到的数据信息。

![图片不存在](media/1b84dc662e019139115f2ef36165433c.png)

![图片不存在](media/bcdef9389dec0ba6e01841b72fac3d41.png)

![图片不存在](media/c48321c0eeb53ac9655502178c3eac88.png)
...
![图片不存在](./media/fc3b1b271cb8d655653a46adeace82a1.png)

对进行MFRC522和IIC初始化。 

![图片不存在](./media/d970dd1cd2207afbce3aa425a03373af.png)
...
BMP388气压传感器是一款新型高性能气压MEMS传感器，非常适用于消费电子无人机、可穿戴设备、智能家居和其他应用中的高度跟踪。BMP388气压传感器是可以检测温度和气压的传感器，是专为移动应用设计的绝对气压传感器。广泛应用于温度检测、大气压强检测、海拔高度检测、室内导航(楼层检测、电梯检测)、户外导航、休闲和运动的应用程序等。
...
| ![图片不存在](media/6a82ee5dd8727ea183dc758cc34e30e0.png)  | ![图片不存在](media/03fd732e12b978158b6b8a8771d76e40.png) |![图片不存在](media/3c2ec52b7de97d4e4d1ff02bad590a30.png) | ![图片不存在](media/fa9ebfb506b2f154108baba98dffa9ee.jpg)    |
| ------------------------ | ------------------------ | ---------------------------- |---------------------------- |
...
![图片不存在](media/aee14abbde2916f420f21109b24ca031.png)

---

**6. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**31_BMP388_Barometric_Pressure**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/bb1192a2fbafe699e44f46c307d10fa4.png)
...
代码上传成功后，拔下USB线断电。按照接线图正确接好模块后再用USB线连接到计算机上电，打开串口监视器，设置波特率为 **<u>9600</u>**，串口监视器显示大气压、海拔和温度。

![图片不存在](media/4baaf68ecadf276b483fdcb68161d1b4.png)

![图片不存在](media/b0f8cb66932a7aa2305f357816b4b616.png)
...
![图片不存在](./media/89c72d32337e4122cbe314c6d0e421c6.png)

初始化BMP388。

![图片不存在](./media/ba5124916b6432cbb1db1c0533119f11.png)

检测并读取温度值，海拔高度值，大气压强值。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**32_PAJ7620U2_Gesture_Recognition**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/e02938a0f4e998427dd5c662cadc05f3.png)
...
![图片不存在](media/3f9cf3905a500d5e763640f7a523da41.png)
...
![图片不存在](./media/71e4b0ba7761cb31715511f9c68a2385.png)

初始化paj7620。

![图片不存在](./media/f49e9abfca24df3d16ee410a02df25e3.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**33_Small_Table_Lamp**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/8c89cc2768db6501a278bde307f5ae78.png)
...
![图片不存在](./media/6fc69bebd540f2101fbdcc6f5972a2d5.png)

SK6812 RGB模块上4个LED灯珠都亮白灯。

![图片不存在](./media/90a96f612ac66ac7109146fc42987870.png)
...
![图片不存在](media/63c074ba92221f66a5238b1d1059c140.png)

---

**4. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**34_Automatic_Induction_Door**"。
...
可以参照第04课，第23课和第33课的代码说明，这里就不多做介绍了。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**35_PIR_Alarm**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/a0485c14409bae2cb03ac3130886f9b5.png)
...
![图片不存在](./media/096120421ce76dd29b6ecf1ee11f8445.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**36_Chameleon**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/318f6f7d6c6960eed1792130c22af4db.png)
...
![图片不存在](media/6e2c66898967aeb30335439730a14586.png)

---

**6. 代码说明**

![图片不存在](./media/9dfce7a904d0931cd9c6cab243c92ee1.png)

![图片不存在](./media/7052dae0104b9a605a5d737227fe9ba5.png)是模除运算符。代码中模除的结果是旋转编码器计数的值与3相除的余数。 

剩余的可以参照第22课的代码说明，这里就不多做介绍了。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**37_Smart_Window**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**38_Night_Lamp**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

<span style="color: rgb(255, 76, 65);">注意：代码中的模拟值的阈值200可以根据实际情况设置。</span>

![图片不存在](./media/3e1fb8a18b817067d5db64226026bd1f.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**39_Pressure_Alarm**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

<span style="color: rgb(255, 76, 65);">注意：代码中的模拟值的阈值400可以根据实际情况设置。</span>

![图片不存在](./media/0bb78734358c63f4c4c96f292f82c8c1.png)
...
![图片不存在](./media/d28388d5d0fce3f9699388668d1b793b.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**40_Temperature-humidity_meter**"。
...
![图片不存在](media/0e59556732cdb54667885c67a327fd46.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**41_TFT_displays_TF_card_pattern**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**42_Collision_Alarm**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/e1559a8039c187e9992c7c2a478a928f.png)
...
![图片不存在](./media/edab7eea436789d8109412a4180b7168.png)
...
在这一实验课程中我们使用红外遥控器上的“ ▲ ”和“ ▼ ”等两个按键控制SK6812 RGB模块上4个LED灯珠分别亮与灭，然后再使用“ ① ”、“ ② ”、“ ③ ”、“ ④ ”、“ ⑤ ”、“ ⑥ ”、“ ⑦ ”、“ ⑧ ”、“ ⑨ ” 等九个按键来控制SK6812 RGB模块上4个LED灯珠分别亮红色灯、绿色灯、蓝色灯、橙色灯、黄色灯、紫色灯、金色灯、粉红色灯、品红色灯的效果。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**43_Intelligent_IR_Control**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/0854c89b9e75c8aa54a7eb63bbaf2f3e.png)
...
![图片不存在](media/26cfa3d4e77ad439945ae7efc743e9eb.png)

---

**6. 代码说明**

可以参照第21课和第26课的代码说明，这里就不多做介绍了。
...
| ![图片不存在](media/6a82ee5dd8727ea183dc758cc34e30e0.png) | ![图片不存在](media/8cc701956c4d1ce4ce501fe370e0fbaa.png) |![图片不存在](media/460ec103035638e948d2b1a272e62178.png)|![图片不存在](media/25f82bd1b415a8203a32392488131376.png) |
| ------------------------ | ------------------------ | ---------------------------- | ------------------------ | 
|主板 x1 |RFID刷卡模块 x1    |舵机驱动模块 x1  |舵机 x1|
| ![图片不存在](media/5cac63a19ecafff8be1ddba2b3fdc015.png)     | ![图片不存在](media/1ff24756e347dcc14db599711e89d922.png) | ![图片不存在](media/fa9ebfb506b2f154108baba98dffa9ee.jpg) |      |
...
![图片不存在](media/946c749c6b8181301a516022c93361e9.png)

---

**4. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**44_Intelligent_access_control**"。

**<span style="background:#ff0;color:#000">特别注意：对于不同的IC卡和钥匙扣，其读取的IC卡和钥匙扣的UID码值可能都不一样。在代码运行前，需要将你自己的IC卡和钥匙扣的UID码值替换程序代码中的UID码（UID码在第30课完成实验可以得知），替换位置如下图所示。</span>**

![图片不存在](media/2f42aa88506bab77623142717e871f08.png)

![图片不存在](./media/52d79f9fe9ec0a5f00f09ade45aa0074.png)
...
代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。上传成功后打开串口监视器，波特率设置为 **<u>9600</u>**。使用正确的IC卡或者钥匙扣刷卡时，串口监视器打印出卡的UID码并提示 “**open the door!**”，同时舵机转动到相应的角度模拟开门的效果。

![图片不存在](media/b2d7d616fec3c61237e55ce8f83d30f6.png)

![图片不存在](media/18eab428ecb107fe4726eeaef7988793.png)
...
| ![图片不存在](media/6a82ee5dd8727ea183dc758cc34e30e0.png) | ![图片不存在](media/20045f4ff8b1171e01b98d503293b771.png) | ![图片不存在](media/0fb16f82134e03f951cb8b8c383577ef.png) | ![图片不存在](media/0af14060bd3d3c37a9cbf480cb3a8db6.png) |
| ------------------------ | ------------------------ | ------------------------ | ------------------------ |
|主板 x1|SK6812 RGB模块 x1     |130电机模块 x1     | 火焰传感器 x1      | 
| ![图片不存在](media/7e77d1498bafcbd3e9f2d40103af3734.png)|![图片不存在](media/1ff24756e347dcc14db599711e89d922.png) |![图片不存在](media/278286c09e3e34f820b4754221ac23aa.png) | ![图片不存在](media/fa9ebfb506b2f154108baba98dffa9ee.jpg) |
|有源蜂鸣器 x1|4P线(反向) x2 | 3P线(反向) x2 | USB线 x1 |
| ![图片不存在](media/aeffa45e36af55436bab7152f894eef9.png)    |   ![图片不存在](media/f5504973a1c7bd3a74e8a9370a3c2358.png)    | ![图片不存在](media/90af020268d3414e4d72f2d6834b218e.png)|          |
...
![图片不存在](media/6a92bd63e1d8bb06727d8c8c5f999b61.png)

---

**4. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**45_Intelligent_Fire_Alarm_Device**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后外接电源供电，上电后，观察实验结果。**

<span style="color: rgb(255, 76, 65);">注意：代码中的模拟值的阈值500可以根据实际情况设置。</span>

![图片不存在](./media/6c9c9708b3eebb29c35a4639db7f4034.png)
...
![图片不存在](./media/a1f9bcd59983657df20a5908eaf7d84a.png)

---

**6. 代码说明**

在代码中我们设置了阈值为500（item < 500），阈值可以根据实际情况更改。当火焰传感器检测到模拟值低于这个阈值时，SK6812 RGB模块上的4个LED闪烁红灯蜂鸣器警报来提醒人们，并且启动风扇来灭火；否则，SK6812 RGB模块上的4个LED不亮蜂鸣器不叫，风扇关闭。风扇的驱动方式请查看上面的第12课。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**46_Corridor_Sensing_Light**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

<span style="color: rgb(255, 76, 65);">注意：代码中的模拟值的阈值可以根据实际情况设置。</span>

![图片不存在](./media/492eaa37f13f554650d5911225d7bb0a.png)
...
![图片不存在](./media/2f04270ebe27eccddfc5152b43059689.png)
...
| ![图片不存在](media/6a82ee5dd8727ea183dc758cc34e30e0.png)     | ![图片不存在](media/0ac210fd8f4528d63cacd1f8acbfabf1.png)     |![图片不存在](media/20045f4ff8b1171e01b98d503293b771.png) |
| ---------------------------- | ---------------------------- | ------------------------ |
...
![图片不存在](media/d566d2ba6f9068d92239a933ba8be27d.png)

---

**4. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**47_Control_Servo_and_LED**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/aa4f6c6ed51a738340c23c2f62825fa7.png)
...
代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。按下自锁按键时，SK6812 RGB模块上的4个LED亮起，然后旋转电位器，舵机则来回缓慢地转动。

![图片不存在](./media/1a1a434980bc5cc67c9cfad3e3c3ba29.png)
...
![图片不存在](media/88eb21588097265d878ff720996bfef3.png)

---

**4. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**48_Weather_Broadcast**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/f10c41236fd50412363492dfacf45cf0.png)
...
代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。上电后，可以在TFT显示屏上看到当前的气压、海拔和气温。

![图片不存在](./media/a4cf72fa5b87e3fd5af899ab6c629241.png)

---

**6. 代码说明**

可以参照第29课和第31课的代码说明，这里就不多做介绍了。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**49_Laser_anti-theft**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/30049f48a98f66a0da72ec8333df23c4.png)
...
![图片不存在](./media/73b94af8e6285cedeea93d108b4f8c30.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**50_Heat_abstractor**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\KidsBlock_代码**</u>”中，我们可以在此路径下打开代码文件''**51_Ultrasonic_radar**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

<span style="color: rgb(255, 76, 65);">注意：代码中的距离阈值可以根据实际情况设置。</span>

![图片不存在](./media/218d79a5042ff3d23749afe83233e766.png)
