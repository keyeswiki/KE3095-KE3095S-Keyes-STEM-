...
![图片不存在](./media/236d86bef83b6bce38b46f299f36bf25.png)


然后参照上述方法依次将剩余的库文件（红色框中的）全部添加完。

![图片不存在](./media/d2c18fb121635114d4942b721c1fbcb3.png)
...
![图片不存在](media/a56b4474c355428c498b5f50d31ec74f.png)
...
![图片不存在](media/777943f6d6ae8976802ecc27b531645e.png)

单击![图片不存在](media/f0779368c84b6430e6331ef8781e5d13.jpg)将代码上传到你所选定的主板上。

![图片不存在](media/46d1397454c518541c2b4df6d60a7e8f.png)

代码上传成功。

![图片不存在](media/6269eeca3fac735c2016eb87e04b3d8e.png)
...
单击![图片不存在](media/f0779368c84b6430e6331ef8781e5d13.jpg)将代码上传到主板上，等待代码上传成功后查看实验结果。
...
激光头模块主要由激光头组成，激光头由发光管芯、聚光透镜、铜可调套筒三部分组成。
...
| 代码                   | 说明                                                         |
| ---------------------- | ------------------------------------------------------------ |
| pinMode(button, INPUT) | 由“ int button = 3; ”知道，定义自锁按键管脚为D3。“INPUT”设置为输入模式。通过pinMode()配置为INPUT必须使用上拉或下拉电阻（我们的模块已经使用上拉电阻R1）。该电阻的目的是在开关断开时将引脚拉至已知状态。通常选择一个4.7K/10K欧姆的电阻，因为它的阻值足够低，可以可靠地防止输入悬空。同时，该阻值也要足够高，以使开关闭合时不会消耗太多电流。如果使用下拉电阻，则当开关断开时，输入引脚将为低电平；当开关闭合时，输入引脚将为高电平。如果使用上拉电阻，则当开关断开时，输入引脚将为高电平；当开关闭合时，输入引脚将为低电平。 |
...
![图片不存在](media/d382fdcfa634b76b1573a2bbf159e9c0.png)
...
![图片不存在](media/75fe0be190cd99608798ab83de8c4146.png)
...
![图片不存在](media/524522ed63dd50bafa4438a972228e12.png)
...
![图片不存在](./media/fa5311b356d4bfba2e58abe8a2226f38.png)
...
![图片不存在](./media/2514071b2aa9f6925ff65c6336119f60.png)

然后将结果打印到串行监视器。

![图片不存在](./media/e00e4d714c1d65cc93aadd077e184fd3.png)
...
![图片不存在](./media/96ba0d21bb35bc57dcbc81ca9510fe7b.png)
...
HC-SR04超声波传感器是可以发送出一种频率很高的人类无法听到的超声波信号，这些超声波的信号碰到障碍物，就会立刻反射回来。在接收到返回的信息之后，根据发射信号和接收信号的时间差，计算出传感器和障碍物的详细距离，和蝙蝠飞行的原理一样。
...
![图片不存在](media/c250ac6c19331a4cef9e2a02c1d4ddfd.png)
...
![图片不存在](media/42c12559c8e9a5da2bf35219be9a4004.jpg) 
...
![图片不存在](media/2cc1c60528b3286116dc6062ae499278.png)
...
![图片不存在](./media/e250fe164b7db8eaf6a3e70faf4ad202.png)

![图片不存在](./media/a687a32761d2198b35ceecf42f8fb7d0.png)
...
RFID（Radio Frequency Identification）：无线射频识别，读卡器由频射模块及高平磁场组成。Tag应答器为待感应设备，此设备不包含电池。他只包含微型集成电路芯片及存储数据的介质以及接收和发送信号的天线。读取tag中的数据，首先要放到读卡器的读取范围内。读卡器会产生一个磁场，因为磁能生电由楞次定律，RFID Tag 就会供电，从而激活设备。
...
上传代码，代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，按下自锁按键，SK6812 RGB模块点亮，再次按下自锁按键(自锁按键按起)，SK6812 RGB模块不亮。循环进行。
...
上传代码，代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。当人体红外热释传感器检测到附近有人经过时，人体红外热释传感器上的红灯灭，有源蜂鸣器发出警报，SK6812 RGB模块4个LED灯珠闪烁。
...
![图片不存在](media/4be29288d2f68f00b9d03c4fcdbf4444.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Arduino_C_代码**</u>”中，我们可以在此路径下打开代码文件''**Pressure_alarm.ino**"。
...
上传代码，代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。为了使实验数据最精准，请将薄膜压力传感器尽量平放。
...
![图片不存在](media/f399b14d76faf806f29de19177a441cd.png)
...
上传代码，代码上传成功后，拔下USB线断电，再用USB线连接到计算机上电，上传成功后可以在主板上的TFT显示屏看到“SD OK”字样，说明主板可以正常读取TF卡上的内容，然后TFT显示屏显示“A6_Value: 5.00 V，“A7_Value: 5.00 V ”字样。之后TFT显示屏循环显示TF卡上的图片文件。

![图片不存在](./media/b11fce7d213c1c50f5f18b3e20c7a2ff.png)
...
![图片不存在](./media/91e42e668018ed4d2daec6b131ffa4b0.png)
...
![图片不存在](./media/d6a4edc0558f47bea287d989b9f11368.png)
...
设置代码，按下“ OK ”键且满足某一条件，点亮模块上4个LED灯珠；再次按下“ OK ”键且满足另一条件，熄灭模块上4个LED灯珠。这个条件我们用 boolean 来实现是最简单方便的，因为 boolean 只有 True 或是 False 两种状态。我们只需要设置按下“ OK ”键的同时 flag 为 true，即可点亮模块上4个LED灯珠；同理，按下“ OK ”键的同时 flag 为 false，熄灭模块上4个LED灯珠。
...
找到红外遥控器，拔出绝缘片，对准红外接收模块的红外接收传感器的接收头。第一次按下红外遥控器上的 “ **OK** ” 键，SK6812 RGB模块上4个LED灯珠被点亮，实现开灯的效果。串口监视器打印出按下的按键值和SK6812 RGB模块上4个LED灯珠亮灯情况：“**<u>BF40FF00  led on</u>**” 。

再次按下红外遥控器上的 “ **OK** ” 键，SK6812 RGB模块上4个LED灯珠熄灭，实现关灯的效果。串口监视器打印出按下的按键值和SK6812 RGB模块上4个LED灯珠亮灯情况：“**<u>BF40FF00  led off</u>**” 。
...
| flag = false        | boolean 数据类型的 flag 赋值为 false ，以便再次按下 “OK”键时满足熄灭SK6812 RGB模块上4个LED灯珠的条件。 |
...
上传代码，代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。上传成功后打开串口监视器，波特率设置为 **<u>115200</u>**。使用正确的IC卡或者钥匙扣刷卡时，串口监视器打印出卡的UID码并提示 “**open the door!**”，同时舵机转动到相应的角度模拟开门的效果。
...
上传代码，代码上传成功后，拔下USB线断电，按照接线图正确接好模块，给主板外接电源供电（将装有电池的电池盒接到主板上）。上电后，打开串口监视器，设置波特率为 **<u>9600</u>**。

串口监视器上打印出火焰的模拟值，当模拟值低于500时，SK6812 RGB模块上的4个LED闪烁蜂鸣器警报来提醒人们，并且启动风扇来灭火。

![图片不存在](./media/95b5da76a919061ad99b564c2f2e06f8.png)
...
![图片不存在](./media/fdd9ffa8545cd97b2962cc699f3e8ffb.png)

![图片不存在](media/a32c6a6f6394ea88da63fa85405f55d7.png)
...
![图片不存在](media/c6607053bffe2b8afef8426d717963e6.png)
...
上传代码，代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。按下自锁按键时，SK6812 RGB模块上的4个LED亮起，然后旋转电位器，舵机则来回缓慢地转动。
...
上传代码，代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。上电后，可以在TFT显示屏上看到当前的气压、海拔和气温。
...
上传代码，代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电。上电后，可以看到激光模块发出的激光正对着光敏传感器模块上的光敏电阻，在光敏传感器模块和激光模块之间用物体挡住激光，发现SK6812 RGB模块上的4个LED灯珠依次闪烁红灯和黄灯，蜂鸣器不断地鸣叫。

![图片不存在](./media/1859fd814077df57d5deb341a3ba6b90.png)
